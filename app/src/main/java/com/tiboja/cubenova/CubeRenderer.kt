...
class CubeRenderer(private val context: Context) : GLSurfaceView.Renderer {
    ...
    @Volatile private var undoQueue = 0        // nb de coups en file qui sont des annulations
    ...
    fun undoMany(n: Int): Int {
        ...
        var count = 0
        repeat(n) {
            val last = history.removeLastOrNull() ?: return@repeat
            undoQueue++
            moveQueue.add(Move(last.axis, last.layer, -last.dir))
            count++
        }
    ...
    fun scramble() = doScramble(java.util.Random())

    /** Mélange reproductible à partir d'une graine (pour le Défi du jour). */
    private fun doScramble(rng: java.util.Random) {
        val n = size
        val half = (n - 1) / 2f
        ...
        repeat(count) {
            ...
            val layer = layers[rng.nextInt(n)]
            val dir = if (rng.nextBoolean()) 1 else -1
            moveQueue.add(Move(axis, layer, dir))
        }
    }

    /** Lance un défi : reconstruit un cube résolu à la taille voulue puis mélange à graine fixe. */
    fun requestChallenge(n: Int, seed: Long) {
        pendingSize = n
        pendingScrambleSeed = seed
        resetRequested = true
    }
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        // Défi : mélange reproductible juste après la reconstruction du cube
